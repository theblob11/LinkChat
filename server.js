...
function sendRoomSettings(roomName) {
...
async function joinRoom(
    ...
    let databaseRoom = await getRoom(roomName);
    ...
    /* EXISTING */
        ...
        */
        ...
        /*
           Locked rooms prevent NEW
           users from joining.
        */

        if (databaseRoom.locked) {
    ...
    if (!rooms.has(roomName)) {
        ...
        rooms.set(roomName, {
            name: databaseRoom.room,
            owner: databaseRoom.owner,
            locked: Boolean(databaseRoom.locked),
            hasPassword:
                Boolean(databaseRoom.password),
            users: new Set()
        });
    }
    ...
    room.name = databaseRoom.room;
    room.owner = databaseRoom.owner;
    room.locked = Boolean(databaseRoom.locked);
    room.hasPassword =
        Boolean(databaseRoom.password);
    ...
    await loadMessages(roomName, ws);
    ...
    ws.on("message", async raw => {
        ...
        try {
            ...
            if (data.type === "join") {
                ...
                const username =
                    cleanText(data.username, 30);

                const roomName =
                    cleanText(data.room, 100);

                const password =
                    typeof data.password === "string"
                        ? data.password.slice(0, 100)
                        : "";
            ...
            if (data.type === "renameRoom") {
                ...
                rooms.set(newName, oldRoom);

                oldRoom.name = newName;

                for (const client of oldRoom.users) {
                    client.room = newName;
                }
            ...
            if (data.type === "transferOwnership") {
                ...
                if (!target) {
                    ...
                    send(ws, {
                        type: "error",
                        message:
                            "That user must be online to receive ownership."
                    });
                ...
                broadcast(ws.room, {
                    ...
                    owner: targetName,
                    message:
                        `${targetName} is now the room owner.`
                });
            ...
            if (data.type === "setLock") {
                ...
                broadcast(ws.room, {
                    ...
                    message:
                        locked
                            ? "The room was locked. New users cannot join."
